...
* Implements methods to increment parts
...
4. Increment version parts
>>> Version(1, 2, 3).inc_major()
Version(2, 0, 0)
>>> Version(1, 2, 3).inc_minor()
Version(1, 3, 0)
>>> Version(1, 2, 3).inc_patch()
Version(1, 2, 4)
"""
...
class Version:
    ...
    * with positional arguments like Version(patch=4); this is equivalent
    ...
    def inc_major(self) -> "Version":
        """Raise major part and set others to zero """
        cls = type(self)
        ver = self.major + 1, 0, 0
        return cls(*ver)

    def inc_minor(self) -> "Version":
        """Raise minor part and set patch part to zero"""
        cls = type(self)
        ver = self.major, self.minor + 1, 0
        return cls(*ver)

    def inc_patch(self) -> "Version":
        """Raise patch part"""
        cls = type(self)
        ver = self.major, self.minor, self.patch + 1
        return cls(*ver)
